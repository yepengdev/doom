...
CJiebaWordWithWeight* ExtractWithWeight(Extractor handle, const char* sentence, size_t len, size_t topn) {
  cppjieba::KeywordExtractor* x = (cppjieba::KeywordExtractor*)handle;
  vector<cppjieba::KeywordExtractor::Word> words;
  x->Extract(string(sentence, len), words, topn);
  CJiebaWordWithWeight* res = (CJiebaWordWithWeight*)malloc(sizeof(CJiebaWordWithWeight) * (words.size() + 1));
  for (size_t i = 0; i < words.size(); i++) {
    size_t offset = words[i].offsets[0];
    res[i].word = sentence + offset;
    res[i].len = words[i].word.size();
  ...
  res[words.size()].word = NULL;
  res[words.size()].len = 0;
  res[words.size()].weight = 0.0;
  return res;
}

void FreeWordsWithWeight(CJiebaWordWithWeight* words) {
  free(words);
}
