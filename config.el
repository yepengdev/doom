...
(defun my/org-live--export ()
  ...
  (let ((output (expand-file-name "index.html" my/org-live--dir)))
    (condition-case err
      ...
      (error
       (message "🍅 Org 实时预览: 导出失败 — %s" (error-message-string err))
       (ding)))))
...
;; ─── 私人配置字节编译（daemon 空闲时，跟踪 SHA-256 哈希）──────

(defvar my/private-config-hash-file
  (expand-file-name "private-config-hashes" doom-local-dir))
...
(defun my/private-config--read-hashes ()
  (ignore-errors
    (with-temp-buffer
      (insert-file-contents my/private-config-hash-file)
      (read (current-buffer)))))

(defun my/private-config--current-hashes (files)
  (mapcar (lambda (f)
            (cons f
...
(defun my/byte-compile-private-config ()
  (interactive)
  (let* ((files (my/private-config--collect))
         (old (my/private-config--read-hashes))
         (result (list)))
    (dolist (file files)
      (let ((hash (secure-hash
                   'sha256 (with-temp-buffer
                             (insert-file-contents file)
                             (buffer-string)))))
        (unless (equal hash (alist-get file old nil nil #'equal))
          (let ((byte-compile-warnings nil))
            (if (byte-compile-file file)
                (push (cons (file-relative-name file doom-user-dir) t) result)
              (push (cons (file-relative-name file doom-user-dir) nil) result))))))
    (let ((success (cl-remove-if-not #'cdr result))
          (failed (cl-remove-if #'cdr result)))
      (when result
        (with-temp-file my/private-config-hash-file
          (prin1 (my/private-config--current-hashes files) (current-buffer))
          (insert "\n")))
      (cond (failed
             (message "Private config: compiled %d, %d failed: %s"
                      (length success) (length failed)
                      (mapconcat #'car (nreverse failed) ", ")))
            (success
             (message "Private config: compiled %d file(s)" (length success)))))))

(add-hook 'doom-after-init-hook
          (lambda ()
            (run-with-idle-timer 1 nil #'my/byte-compile-private-config)))
