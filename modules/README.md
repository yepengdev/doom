...
| `jieba-module.so` | `jieba-module.c` + `cjieba/libjieba.a` | 结巴中文分词 + TF-IDF 关键词提取（需要 g++ C++11） |
...
依赖：`gcc`、`emacs-module.h`（Emacs 30）、`libffi`、`libnotify`、`g++`（jieba 需要 C++11）。
...
所有模块编译为 `.so`，由 `my/*--ensure` 函数通过 `module-load` 延迟加载（首次调用对应的
Lisp 符号时才触发）。每个模块的 `emacs_module_init` 注册函数并 `provide` 一个
feature，`my/*--ensure` 函数检查该 feature 后再加载，避免重复加载和启动阻塞。

C 模块可通过 `cli.el` 的 `doom-after-sync-hook` 在 `doom sync` 时自动构建。详情见
`cli.el` 和 `c-modules/Makefile`。
